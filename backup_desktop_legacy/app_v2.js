...
const materialDetailsMap = {
    ...
    "Mesh 230GSM": {
        ...
        badges: ["Image/Material/Hot Sale.webp"],
...
window.closeV3Modal = function(modalKey) {
    ...
    if (!anyActive && (!lightboxOverlay || !lightboxOverlay.classList.contains('active'))) {
...
function renderV3Material() {
    ...
    materials.forEach(mat => {
        ...
        html += `
            <div class="v3-material-card" onclick="openV3LightboxDirect('${imgEsc}', '${titleEsc}', 'material')">
...
function renderV3Cutting() {
    ...
    cuttings.forEach(c => {
        ...
        const titleEsc = escapeJsAttr(title);
        const imgEsc = escapeJsAttr(imgSrc);

        html += `
            <div class="v3-spec-card" onclick="openV3LightboxDirect('${imgEsc}', '${titleEsc}', 'cutting')">
...
function renderV3Neck() {
    ...
    necks.forEach(n => {
        ...
        html += `
            <div class="v3-spec-card" onclick="openV3LightboxDirect('${imgEsc}', '${titleEsc}', 'neck')">
                <div class="v3-spec-img-wrap">
                    <img src="${imgSrc}" alt="${title}" class="v3-spec-img" loading="lazy">
                </div>
                <div class="v3-spec-footer">
...
function renderV3Printing(tab) {
    ...
    if (tab === 'guide') {
        ...
        const imgEsc = escapeJsAttr(imgSrc);
        html = `
            <div class="v3-spec-card" style="grid-column: 1 / -1;" onclick="openV3LightboxDirect('${imgEsc}', 'Placement Guide', 'printing')">
                <div class="v3-spec-img-wrap wide">
                    <img src="${imgSrc}" alt="Placement Guide" class="v3-spec-img" loading="lazy">
                </div>
                <div class="v3-spec-footer">
                    <span class="v3-spec-title">Official Placement Guide</span>
                </div>
            </div>
        `;
    } else {
        items.forEach((item, index) => {
            ...
            const labelEsc = escapeJsAttr(label);
            const imgEsc = escapeJsAttr(imgSrc);

            html += `
                <div class="v3-spec-card" onclick="openV3LightboxDirect('${imgEsc}', '${labelEsc}', 'printing')">
                    <div class="v3-spec-img-wrap">
...
function renderV3SizeChart(tab) {
    ...
    items.forEach(item => {
        ...
        html += `
            <div class="v3-spec-card" onclick="openV3LightboxDirect('${imgEsc}', '${titleEsc}', 'sizechart')">
...
window.switchV3LightboxSlide = function(direction) {
    ...
    if (direction === 'next') {
        v3LightboxIndex = (v3LightboxIndex + 1) % v3LightboxImages.length;
    } else {
        v3LightboxIndex = (v3LightboxIndex - 1 + v3LightboxImages.length) % v3LightboxImages.length;
    }
    const lightboxImg = document.getElementById('lightboxImg');
    if (lightboxImg) {
        lightboxImg.src = v3LightboxImages[v3LightboxIndex];
    }
    updateLightboxDots(v3LightboxImages, v3LightboxIndex);
};
...
window.openV3LightboxFromCard = function(ref, sourceContext, fallbackImgSrc) {
    const allProducts = [
        ...
        ...(typeof catalogProductsSpecs !== 'undefined' ? catalogProductsSpecs : [])
    ];

    const cleanRef = (ref || '').trim();
    const productObj = allProducts.find(p => p && p.id && (p.id === cleanRef || p.id.toLowerCase() === cleanRef.toLowerCase()));
    let imagesArray = [];

    if (productObj) {
        const rawList = [];
        if (productObj.image) rawList.push(productObj.image);
        if (productObj.images && Array.isArray(productObj.images)) {
            productObj.images.forEach(img => {
                if (img && !rawList.includes(img)) rawList.push(img);
            });
        }
        imagesArray = rawList.map(img => resolveImagePath(img));
    }

    if (imagesArray.length === 0 && fallbackImgSrc) {
        imagesArray = [resolveImagePath(fallbackImgSrc)];
    }

    if (imagesArray.length === 0) return;

    window.currentV3Ref = cleanRef || 'Preview';
    v3LightboxImages = imagesArray;
    v3LightboxIndex = 0;
    ...
    if (lightboxImg) lightboxImg.src = v3LightboxImages[0];
    ...
    if (refDisplay) {
        refDisplay.innerText = ref.startsWith('Design') || ref.startsWith('WC') ? ref : `Design: ${ref}`;
    ...
    if (btnStack) btnStack.style.display = 'flex';
...
window.openV3LightboxDirect = function(imgSrc, title, type) {
    window.currentV3Ref = title;
    v3LightboxImages = [imgSrc];
    v3LightboxIndex = 0;
    ...
    const refDisplay = document.getElementById('lightboxRefDisplay');
    if (refDisplay) {
        refDisplay.innerText = title;
        refDisplay.style.display = 'block';
    }

    const disclaimerBox = document.getElementById('lightboxDisclaimerBox');
    if (disclaimerBox) disclaimerBox.style.display = (type === 'collection' || type === 'event') ? 'block' : 'none';

    const btnStack = document.getElementById('lightboxBtnStack');
    if (btnStack) {
        btnStack.style.display = (type === 'collection' || type === 'event') ? 'flex' : 'none';
    }
...
function updateLightboxDots(imgs, activeIdx) {
    ...
    if (imgs && imgs.length > 1) {
        ...
        for (let i = 0; i < imgs.length; i++) {
            dotsHtml += `<span class="v3-dot ${i === activeIdx ? 'active' : ''}"></span>`;
        }
...
window.copyCurrentMockupImage = async function() {
    const lang = localStorage.getItem('thirtyone_lang') || 'en';
    const successMsg = (i18nTranslations[lang]?.v3_toast_copied) || 'Mockup image copied to clipboard! Paste it directly in WhatsApp.';
